...
class Ui_product(object):
    ...
    def on_product_type_button_clicked(self):
        ...
        for color_button in self.page_product_color.findChildren(QtWidgets.QPushButton):
    ...
    def get_colors_for_product(product_name):
        if product_name ==urun_listesi[product_name] :
    ...
    def renk_secimi(self,urun):
            for color in urun_listesi[urun]["renkler"]:
                print("********kalemin color************" + color)
                button = QtWidgets.QPushButton(color)
                button.setObjectName(color)
                self.h_layout.addWidget(button)
                print("2.adım  "+button.objectName())
            for button in self.page_product_type.findChildren(QtWidgets.QPushButton):
                button.clicked.connect(self.on_product_type_button_clicked)
            self.page_product_type.setLayout(self.h_layout)
            self.stackedWidget.addWidget(self.page_2_product_color)
